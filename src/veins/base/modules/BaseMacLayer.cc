...
void BaseMacLayer::initialize(int stage)
{
    ...
    if(stage==0)
    {
        ...
        myMacAddr = intuniform(0,0xFFFFFFFE);
        registerInterface();
    }
...
InterfaceEntry *BaseMacLayer::createInterfaceEntry()
{
    ...
    e->setIPv4Data(ipv4data);
    // address
    MACAddress address((uint64)myMacAddr);
    e->setMACAddress(address);
    e->setInterfaceToken(address.formInterfaceIdentifier());

    e->setMtu(par("mtu").longValue());
    e->setNetworkLayerGateIndex(0);
    // capabilities
    e->setBroadcast(true);
    e->setMulticast(true);
